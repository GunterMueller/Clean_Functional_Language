...
backEndInterface outputFileName commandLineArgs listTypes typesPath predef_symbols syntaxTree=:{fe_icl,fe_components,fe_dcls} main_dcl_module_n
	...
	| be_parse_command_args_result<0
		...
		=	(backEndFiles == 0 && False, heaps, errorFile, outFile, files)
	...
	| fail_success_or_use_clean_file_io==2
		...
		| fopen_ok
			...
			| backEndFiles==0
				...
				= (success, heaps, errorFile, outFile, files)
				# (_,files) = fclose abc_file files
				# (_,files) = fremove outputFileName files
				= (False, heaps, errorFile, outFile, files)
			# hp_type_heaps=heaps.hp_type_heaps
			  attrHeap=hp_type_heaps.th_attrs		
			  backEnd = BECloseFiles backEnd
			  (attrHeap, outFile, backEnd)
			  ...
			  backEndFiles = BEFree backEnd backEndFiles
			  heaps = {heaps & hp_type_heaps = {hp_type_heaps  & th_attrs = attrHeap}}	
			= (backEndFiles==0, heaps, errorFile, outFile, files)
		# (success, backEnd)
		...
		# backEndFiles = BEFree backEnd backEndFiles
		# heaps = {heaps & hp_type_heaps = {hp_type_heaps  & th_attrs = attrHeap}}	
		=	(backEndFiles == 0 && success, heaps, errorFile, outFile, files)
