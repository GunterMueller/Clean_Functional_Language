...
where
	(<#<) pp_state=:{file,indent_level,last_character_written_was_nl} '\n'
		#! file
			= case False /*last_character_written_was_nl*/ of
				True
				//	#! (_,file)
				...
				_	
					...
					# file
						= write_indent indent_level file
					-> file
		= { pp_state & file = file, last_character_written_was_nl = True }
	where 
		write_indent 0 file
			= file
		write_indent i file
...
instance <#< FunDef
where
	(<#<) pp_state {fun_symb,fun_index,fun_body=ParsedBody bodies} = pp_state <#< fun_symb <#< '.' <#< fun_index <#< '\n' <#< bodies 
